...
async function loadProjects() {
  ...
  // Debug: start fetch
  console.log('⎡DEBUG⎦ Fetching repos.json…');
  const resp = await fetch('repos.json');  // o '/repos.json' a seconda del tuo setup
  console.log('⎡DEBUG⎦ fetch OK, status =', resp.status);

  let repos;
  try {
    repos = await resp.json();
    console.log('⎡DEBUG⎦ Parsed repos:', repos.length, repos);
  } catch (parseError) {
    console.error('⎡DEBUG⎦ JSON parse error:', parseError);
    container.innerHTML = `<div class="terminal-line gruv-red">JSON parse error: ${parseError.message}</div>`;
    return;
  }
  ...
  // Debug: verify DOM before insertion
  console.log('⎡DEBUG⎦ Inserting project cards into container');
  ...
  // Debug: final count
  console.log('⎡DEBUG⎦ Final card count:', container.childElementCount);
}
